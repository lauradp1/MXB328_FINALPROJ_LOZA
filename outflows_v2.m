...
t_diff = diff(t_vals);
index = 365/t_diff(end);
outflow_year = avg_outflow(end - round(index):end);
outflow_50 = repmat(outflow_year,50,1);
outflow_50 = reshape(outflow_50',1,size(outflow_50,1)*size(outflow_50,2));
t_vals_50 = (t_vals(end)+t_diff(end)):t_diff(end):50*365+t_vals(end);
avg_outflow_new = [avg_outflow,outflow_50];
%%
C0 = 1.78; %g/L;
D = 4e-9; %m^2/s
lambda = 3e-9; %s^-1
C_crit = 1e-6; %g\L
V = avg_outflow_new.*(1/(24*60*60)); %0.0108 - 0.0166
